...
void Game::Render()
{
...

	{
		...
		// IA
		_deviceContext->IASetVertexBuffers(0, 1, _vertexBuffer->GetBuffer().GetAddressOf(), &stride, &offset);
		_deviceContext->IASetIndexBuffer(_indexBuffer->GetBuffer().Get(), DXGI_FORMAT_R32_UINT, 0);
		_deviceContext->IASetInputLayout(_inputLayout->GetInputLayout().Get());
		_deviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

		// VS
		_deviceContext->VSSetShader(_vertexShader->GetShader().Get(), nullptr, 0);
		_deviceContext->VSSetConstantBuffers(0, 1, _constantBuffer->GetBuffer().GetAddressOf());
		...
		// PS
		_deviceContext->PSSetShader(_pixelShader->GetShader().Get(), nullptr, 0);
		_deviceContext->PSSetShaderResources(0, 1, _texture1->GetSRV().GetAddressOf());
		...
		_deviceContext->DrawIndexed((UINT)_geometry->GetIndexCount(), 0, 0);
	}
